...
function aggregateInventoryCharts(buildings, rateSummary) {
  ...
  const vacancyTypeBars = Object.entries(vacancyTypeBuckets).filter(([, v]) => v > 0).map(([name, sf]) => ({ name, sf }));
  let rateHistogram = [];
...
function aggregateVacancyBySubmarket(buildings) {
  ...
  return [...map.values()].map((r) => ({ ...r, total: r.direct + r.sublease })).filter((r) => r.total > 0).sort((a, b) => b.total - a.total);
}
function aggregateVacancyByGeneration(buildings) {
  let g1 = 0, g2 = 0;
  for (const b of buildings) {
    if (b.status !== "Existing") continue;
    const avail = Number(b.sf_available) || 0;
...
function aggregateVacancyRateBySubmarket(buildings) {
  const map = /* @__PURE__ */ new Map();
  for (const b of buildings) {
    if (b.status !== "Existing") continue;
    const sm = b.submarket || "Unknown";
  ...
  return [...map.values()].filter((r) => r.existingSf > 0).map((r) => ({ name: r.name, direct: r.direct / r.existingSf * 100, sublease: r.sublease / r.existingSf * 100 })).sort((a, b) => b.direct + b.sublease - (a.direct + a.sublease));
}
var SIZE_TRANCHES = [
  { label: "\u2264100K", min: 0, max: 1e5 },
  { label: "100\u2013250K", min: 1e5, max: 25e4 },
  { label: "250\u2013500K", min: 25e4, max: 5e5 },
  { label: "500K\u20131M", min: 5e5, max: 1e6 },
  { label: "1M+", min: 1e6, max: Infinity }
];
function aggregateBuildingSizeDistribution(buildings, { existingOnly = false } = {}) {
...
var VAC_SIZE_TRANCHES = [
  { label: "< 50K", min: 0, max: 5e4 },
  { label: "50\u2013100K", min: 5e4, max: 1e5 },
  { label: "100\u2013250K", min: 1e5, max: 25e4 },
  { label: "250\u2013500K", min: 25e4, max: 5e5 },
  { label: "500K\u20131M", min: 5e5, max: 1e6 },
  { label: "1M+", min: 1e6, max: Infinity }
...
function aggregateTopOwners(buildings, { limit = 10 } = {}) {
  const map = /* @__PURE__ */ new Map();
  for (const b of buildings) {
...
var LEASE_TYPE_ORDER = ["New", "Renewal", "Expansion", "Sublease", "Owner User"];
function aggregateLeasingByQuarter(deals) {
  const map = /* @__PURE__ */ new Map();
...
function aggregateLeasingBySubmarket(deals) {
  const map = /* @__PURE__ */ new Map();
  for (const d of deals) {
...
function aggregateLeasingByType(deals) {
  const map = /* @__PURE__ */ new Map();
  for (const d of deals) {
...
function aggregateLeasingYoYBySubmarket(deals) {
  ...
  return [...map.values()].filter((r) => r.sfThis > 0 || r.sfLast > 0).sort((a, b) => b.sfThis + b.sfLast - (a.sfThis + a.sfLast));
}
function aggregateTopTenants(deals, { limit = 10 } = {}) {
  const map = /* @__PURE__ */ new Map();
  for (const d of deals) {
...
function aggregateTopBuildingsByLeasing(deals, { limit = 10 } = {}) {
  const map = /* @__PURE__ */ new Map();
  for (const d of deals) {
...
function aggregateStatusBySubmarket(buildings, status) {
  return bySubmarketSf(buildings.filter((b) => b.status === status), "total_sf");
}
function aggregateDeliveriesBySubmarket(deliveries) {
  return bySubmarketSf(deliveries, "total_sf_at_delivery");
}
function quarterKey(s) {
...
function aggregateDeliveriesByQuarter(deliveries) {
  const map = /* @__PURE__ */ new Map();
...
function existingAgg(buildings) {
  ...
  for (const b of ex) {
    ...
    direct += Math.max(0, a - s);
    sub += s;
  }
  return { existingCount: ex.length, existingSf, direct, sub, vacantCount, vacantSf: direct + sub };
}
var statusSf = (b, s) => b.filter((x) => x.status === s).reduce((a, x) => a + (Number(x.total_sf) || 0), 0);
var statusCount = (b, s) => b.filter((x) => x.status === s).length;
var daysOnMarket = (b) => b.vacancy_start_date ? Math.floor((Date.now() - new Date(b.vacancy_start_date).getTime()) / 864e5) : null;
var normPct = (v) => v == null ? null : Number(v) > 1 ? Number(v) / 100 : Number(v);
function deliveryPrePct(d) {
...
var deliveryPreSf = (d) => {
  const p = deliveryPrePct(d);
  return p != null ? Math.round((Number(d.total_sf_at_delivery) || 0) * p) : 0;
};
export {
  aggregateBuildingSizeDistribution,
  aggregateDeliveriesByQuarter,
  aggregateDeliveriesBySubmarket,
  aggregateInventoryCharts,
  aggregateLeasingByQuarter,
  aggregateLeasingBySubmarket,
  aggregateLeasingByType,
  aggregateLeasingYoYBySubmarket,
  aggregateOwnership,
  aggregateStatusBySubmarket,
  aggregateTopBuildingsByLeasing,
  aggregateTopOwners,
  aggregateTopTenants,
  aggregateUcBySize,
  aggregateVacancyByGeneration,
  aggregateVacancyBySize,
  aggregateVacancyBySubmarket,
  aggregateVacancyRateBySubmarket,
  daysOnMarket,
  deliveryPrePct,
  deliveryPreSf,
  existingAgg,
