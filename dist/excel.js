...
function fmtSf(n) {
  ...
  if (Math.abs(num2) >= 1e9) return (num2 / 1e9).toFixed(1) + "B";
  if (Math.abs(num2) >= 1e6) return (num2 / 1e6).toFixed(1) + "M";
  if (Math.abs(num2) >= 1e3) return (num2 / 1e3).toFixed(0) + "K";
...
var CP35_SECTIONS = [
  {
    key: "proposals_out",
    title: "SECTION I: ACTIVE DEALS - PROPOSALS OUT",
  ...
  {
    key: "prospects",
    title: "SECTION III: PROSPECT TRACKING",
...
function buildMarketingReport({ property = {}, sections = [] } = {}) {
...
function supplyRowXl(cs, b, mark) {
  ...
  return [
    ...
    b.submarket || "",
    num(b.total_sf),
    (Number(b.sf_available) || 0) > 0 ? num(b.sf_available) : "",
    b.status || "",
    gen,
    rateCell(cs.ratesByBuilding?.[b.id]?.rate),
    b.owner || ""
  ];
}
...
export {
  CP35_SECTIONS,
  addCompetitiveSetSheets,
  addMarketSheets,
  addMarketingReportSheet,
  addTableSheet,
  buildCombinedReport,
