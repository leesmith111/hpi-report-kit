// Canonical DFW submarket rollups — the DEFAULT normalizer used by /select and
// by agency-hub. Vendored from ftw-cre-platform/src/utils/submarketCanonical.js
// (the submarket portion only; FTW keeps its fuller micro-market / market-scope
// apparatus locally and injects its own `toCanonicalSubmarket` into /select).
//
// Maps historical / granular submarket strings to one canonical submarket so
// comp/building submarkets match consistently. Non-DFW values fall through
// unchanged.
const CANONICAL_MAP = {
  // NE Tarrant / Alliance
...
export { CANONICAL_MAP };

export function toCanonicalSubmarket(s) {
  if (s == null) return s;
