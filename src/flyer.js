// Property-flyer primitives — the HPI house marketing-flyer language (portrait
// 8.5×11, per-property accent branding). Layout transcribed from the classic
// house template (exemplar: "Mark IV Parkway flyer.pdf" — cover w/ hero photo +
// name chip + FOR LEASE banner + features/locator, PROPERTY OVERVIEW spec grid,
// SITE PLAN / AMENITIES / INGRESS image pages, TREC IABS back page; every page
// wears the photo-band header + LEASED BY / CONTACT / OWNED BY footer).
// Browser-only, pptxgenjs dynamically imported like ./pptx.js. Unlike the pitch
...
const BASE = {
  ACCENT: '0071AD',       // per-property brand color (Mark IV blue default)
  ACCENT_DARK: '164A73',  // name-chip / darker companion
  INK: '3F4448',          // body text dark gray
  GRAY: '787E85',         // secondary text / light title word
  LIGHT: 'F2F5F8',        // light panel fill
  RULE: 'D4D8DC',         // spec-row underrules
  WHITE: 'FFFFFF',
  FONT: 'Montserrat',     // referenced by name (house flyers' geometric sans); falls back if absent
  MX: 0.55,               // side margin
  HEADER_H: 1.55,         // interior-page photo band
  FOOTER_Y: 10.02,
};

// Build a theme: flyerTheme({ ACCENT: 'E87722' }) → tokens for an orange-branded property.
export function flyerTheme(overrides = {}) {
...
// Split a [label, value] spec list into two visually balanced columns, weighting
// two-line values (long text wraps) so both columns end near the same height.
export function splitSpecs(specs, { charsPerLine = 26 } = {}) {
  const weight = (s) => 1 + (String(s?.[1] ?? '').length > charsPerLine ? 1 : 0);
  const total = specs.reduce((a, s) => a + weight(s), 0);
  const left = [];
  let acc = 0;
  for (const s of specs) {
    if (acc < total / 2) { left.push(s); acc += weight(s) } else break;
  }
...
function text(slide, T, x, y, w, h, str, { size = 10, bold = false, color = T.INK, align = 'left', valign = 'middle', wrap = true, italic = false, charSpacing, hyperlink, lineSpacingMultiple = 1 } = {}) {
  slide.addText([{ text: String(str ?? ''), options: { fontFace: T.FONT, fontSize: size, bold, italic, color, charSpacing, hyperlink } }],
    { x, y, w, h, align, valign, wrap, margin: 0, lineSpacingMultiple });
}

// Two-tone section title: "PROPERTY" bold ink + " OVERVIEW" light gray.
export function addSectionTitle(slide, T, boldWord, lightWord, { x = 0.6, y, size = 21 } = {}) {
  slide.addText([
...
// ---------------------------------------------------------------------------
// createFlyerDeck — portrait 8.5×11 deck.
// ---------------------------------------------------------------------------
export async function createFlyerDeck({ author = 'HPI' } = {}) {
...
// ---------------------------------------------------------------------------
// Footer band: LEASED BY (logo) · CONTACT (rich lines) · OWNED BY (logo).
// contacts: [{ name, phone, email }]. Logos = data URLs (optional).
// ---------------------------------------------------------------------------
export function addFlyerFooter(slide, T, { leasedByLogo, contacts = [], ownedByLogo, leasedByLabel = 'LEASED BY', contactLabel = 'CONTACT', ownedByLabel = 'OWNED BY' } = {}) {
  const y = T.FOOTER_Y;
...
// Name chip: accent-dark block with the (multi-line) property name in white.
function nameChip(slide, T, name, { x, y, w, h, size = 13 }) {
  slide.addText(String(name).toUpperCase(), {
    x, y, w, h, shape: 'rect', fill: { color: T.ACCENT_DARK }, line: { type: 'none' },
    fontFace: T.FONT, fontSize: size, bold: true, color: T.WHITE, align: 'center', valign: 'middle',
    margin: [4, 6, 4, 6], lineSpacingMultiple: 1.05,
  });
}

// Address bar: accent strip with the "ADDRESS | CITY, STATE ZIP" line.
function addressBar(slide, T, address, { x, y, w, h = 0.32, size = 10 }) {
  slide.addText(String(address).toUpperCase(), {
    x, y, w, h, shape: 'rect', fill: { color: T.ACCENT }, line: { type: 'none' },
    fontFace: T.FONT, fontSize: size, bold: false, color: T.WHITE, align: 'center', valign: 'middle', margin: 0, wrap: false,
  });
}

// ---------------------------------------------------------------------------
// addFlyerPage — interior-page chrome: photo band header + name chip + address
// bar + footer. Returns the slide; content area starts at y ≈ 1.85.
// ---------------------------------------------------------------------------
export function addFlyerPage(deck, { theme, name, address, headerPhoto, footer }) {
  const T = theme;
...
// ---------------------------------------------------------------------------
// addFlyerCover — full-bleed hero photo, big name chip, FOR LEASE banner,
// address bar, PROPERTY FEATURES bullets (left) + locator map (right), footer.
// ---------------------------------------------------------------------------
export function addFlyerCover(deck, { theme, name, address, photo, saleLine = 'FOR LEASE', subLine, features = [], featuresTitle = ['PROPERTY', 'FEATURES'], locatorImage, footer }) {
  const T = theme;
  const s = deck.addSlide();
  s.background = { color: T.WHITE };
  const photoH = 6.15;
...
// ---------------------------------------------------------------------------
// addFlyerSpecsPage — "PROPERTY OVERVIEW": two balanced columns of
// accent-label / ink-value rows with thin underrules; optional photo below.
// specs: [[label, value], ...]
// ---------------------------------------------------------------------------
export function addFlyerSpecsPage(deck, { theme, name, address, headerPhoto, footer, title = ['PROPERTY', 'OVERVIEW'], specs = [], photo }) {
  const T = theme;
...
// ---------------------------------------------------------------------------
// addFlyerImagePage — SITE PLAN / AMENITIES MAP / INGRESS-EGRESS style page:
// two-tone title, optional CTA chip (hyperlinked), big contained image, and an
// optional accent bullet-card overlaid bottom-left (the ingress-page pattern).
// ---------------------------------------------------------------------------
export function addFlyerImagePage(deck, { theme, name, address, headerPhoto, footer, title, image, cta, bullets = [] }) {
  const T = theme;
  const s = addFlyerPage(deck, { theme, name, address, headerPhoto, footer });
  addSectionTitle(s, T, title[0], title[1], { y: 1.92 });
  if (cta?.text) {
    s.addText(String(cta.text).toUpperCase(), {
      x: 5.35, y: 1.95, w: 2.8, h: 0.38, shape: 'rect', fill: { color: T.ACCENT }, line: { type: 'none' },
      fontFace: T.FONT, fontSize: 11, bold: false, color: T.WHITE, align: 'center', valign: 'middle', margin: 0,
...
// ---------------------------------------------------------------------------
export function addFlyerDisclosurePage(deck, { theme, image }) {
  const T = theme || BASE;
  const s = deck.addSlide();
  s.background = { color: T.WHITE };
